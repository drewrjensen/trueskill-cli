...
def list_players():
    if not db.players:
        ...
        return
    sorted_players = sorted(p.name for p in db.players)
...
def show_rankings_for_date(date_str):
    ...
    # Find the most recent date <= date_str
    c.execute(
        """
    SELECT MAX(date)
    FROM player_days
  ...
  """,
    ...
    # Fetch the rankings for that date
    c.execute(
        """
  ...
  """,
    ...
    pad_width = len(str(len(players)))
    longest_name = max(len(name) for name, _, _ in players)

    print(f"Rankings for {latest_date} (closest to requested: {date_str}):")
    for idx, (name, mu, sigma) in enumerate(players, start=1):
        print(
...
def add_match(input_str, datetime_override=None):
    try:
        ...
        db.regenerate_player_days_up_to(match_date, db.players, db.matches)
